...
**核心：** 遵循编程语言的简洁哲学。绝不进行不必要的抽象，绝不引入非必需的依赖。
- **1.1 (YAGNI):** 只实现明确要求的功能。
- **1.2 (标准库优先):** 必须优先使用语言的标准库和内置数据类型。
- **1.3 (反过度工程):** 简单的函数和数据结构优于复杂的函数和数据结构。
...
**核心：** 代码的首要目的是让人类易于理解。
- **3.1 (错误处理):** **不可协商**：要求显式捕获异常，避免程序静默崩溃。
- **3.2 (无全局变量):** 绝不允许使用全局变量来传递状态，所有依赖必须通过函数参数或结构体成员显式注入。
- **3.3 (模块化与命名空间):** 使用语言的模块系统组织代码，保持清晰的命名空间。
- **3.4 ("{{LANGUAGE}}ic" 代码文化):** 用最符合{{LANGUAGE}}风格的方式编写代码。
...
**注意**: 本宪法模板由 cc-tool 自动生成，项目团队应根据实际需求进行调整和完善。