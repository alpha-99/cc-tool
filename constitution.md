...
**核心：** 遵循Python语言的“语法简洁”哲学。绝不进行不必要的抽象，绝不引入非必需的依赖。
- **1.1 (YAGNI):** 只实现`spec.md`中明确要求的功能。
- **1.2 (标准库优先):** 必须优先使用内置数据类型，如列表、字典、集合等。
...
**核心：** 代码的首要目的是让人类易于理解。
- **3.1 (错误处理):** **不可协商**：要求显式捕获异常，避免程序静默崩溃，对应“错误绝不应该被默默忽略”。
- **3.2 (无全局变量):** 绝不允许使用全局变量来传递状态，所有依赖必须通过函数参数或结构体成员显式注入。
- **3.3 (模块化与命名空间):** import 机制让代码组织清晰，符合“命名空间是个绝妙的主意”。
- **3.4 (“Pythonic” 代码文化):** 用最符合python风格的方式编写代码（如用迭代器而不是下标循环）。
